...
if path.exists(snakemake.output[0][:-18]+"Assemblies/"+snakemake.params["Lineage"]):  #If the Lineage already exists do not download it again (for later quality controls)
    os.system("python3.6 /run_BUSCO.py -i " + snakemake.input[0] + " -o BUSCO -m geno -l " + snakemake.output[0][:-18]+"Assemblies/"+ snakemake.params["Lineage"] + " -c "+str(snakemake.threads)+" -sp "+snakemake.params["SP"])
    ...
    os.system("python3.6 /run_BUSCO.py -i "+snakemake.input[0]+ " -o BUSCO -m geno -l " +snakemake.input[0][:-19]+snakemake.params["Lineage"]+ " -c "+str(snakemake.threads)+" -sp "+snakemake.params["SP"])

os.system("mv run_BUSCO "+snakemake.output[1])
